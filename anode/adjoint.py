...
class Checkpointing_Adjoint(torch.autograd.Function):
    ...
    def forward(ctx, *args):
        z0, func, flat_params, options, reverse = args[0], args[1], args[2], args[3], args[4]
        ctx.func = func

        with torch.no_grad():
            ans = odesolver(func, z0, options, reverse)
    ...
    def backward(ctx, grad_output):
        ...
        return out1[0], None, flatten_params_grad(out2, func.parameters()), None, None



def odesolver_adjoint(func, z0, options = None, reverse = False):

    flat_params = flatten_params(func.parameters())
    zs = Checkpointing_Adjoint.apply(z0, func, flat_params, options, reverse)
